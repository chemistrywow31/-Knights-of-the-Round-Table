...
- Applies to: All agents (Butler has additional coordinator responsibilities)

## Rule Content

### Coordinator Must Include Worklog Paths in Task Dispatch

When Butler dispatches a task to any member via the Task tool, the dispatch must include:
...
Butler must not pass full conversation content inline. Pass paths; let the member read what it needs.
...
When a member completes a task and returns results to Butler, the return must be a structured summary:
...
- **DONE_WITH_CONCERNS**: Task completed, but Butler must be aware of an issue. List each concern with recommended action.
...
Butler must handle each status before proceeding. Do not re-dispatch a BLOCKED task without addressing the blocker.
...
- Butler's context accumulates only summaries, not execution details
- Parallel roundtable participants cannot see each other's in-progress outputs

Butler must not perform advisory work inline. All member-level analysis goes through Task dispatch.
...
- At conversation end, Butler writes consolidated memory; subsequent conversations read from memory, not from the current conversation context

## Violation Determination

- Butler dispatches a Task without specifying the relevant profile or memory paths → Violation
- Butler passes full conversation history inline instead of referencing memory paths → Violation
- Member returns unstructured output exceeding 500 words without worklog reference → Violation
- Butler performs advisory analysis inline instead of dispatching a Task → Violation
- A BLOCKED task is re-dispatched without resolving the stated blocker → Violation

## Exceptions

- During initial onboarding (profile-interview), Butler may include more inline context because no memory files exist yet to reference
- Brief clarification exchanges (under 100 words) between Butler and a member do not require structured return format
